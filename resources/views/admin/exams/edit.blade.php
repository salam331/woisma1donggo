                ...
                <div class="p-6 lg:p-8 bg-white border-b border-gray-200">
                    <form method="POST" action="{{ route('admin.exams.update', $exam) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="school_class_id" id="school_class_id" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                ...
                                @foreach($schoolClasses as $schoolClass)
                                    <option value="{{ $schoolClass->id }}" {{ old('school_class_id', $exam->school_class_id) == $schoolClass->id ? 'selected' : '' }}>
                                        {{ $schoolClass->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <x-text-input id="start_time" class="block mt-1 w-full" type="time" step="60" name="start_time" :value="old('start_time', date('H:i', strtotime($exam->start_time)))" required />
                            <x-input-error :messages="$errors->get('start_time')" class="mt-2" />
                        </div>

                        <div class="mb-4">
                            <x-input-label for="end_time" :value="__('Waktu Selesai')" />
                            <x-text-input id="end_time" class="block mt-1 w-full" type="time" step="60" name="end_time" :value="old('end_time', date('H:i', strtotime($exam->end_time)))" required />
                            <x-input-error :messages="$errors->get('end_time')" class="mt-2" />
                        </div>

                        <div class="mb-4">
                            <x-input-label for="total_score" :value="__('Skor Total')" />
                            <x-text-input id="total_score" class="block mt-1 w-full" type="number" step="0.01" name="total_score" :value="old('total_score', $exam->total_score)" required />
                            <x-input-error :messages="$errors->get('total_score')" class="mt-2" />
                        </div>

                        <div class="mb-4">
                            <x-input-label for="description" :value="__('Deskripsi')" />
                            <textarea id="description" name="description" rows="4" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('description', $exam->description) }}</textarea>
                        ...
                        <div class="mb-4">
                            <label for="publish" class="inline-flex items-center">
                                ...
                                <input id="publish" type="checkbox" class="rounded dark:bg-gray-900 border-gray-300 dark:border-gray-700 text-indigo-600 shadow-sm focus:ring-indigo-500 dark:focus:ring-indigo-600 dark:focus:ring-offset-gray-800" name="publish" value="1" {{ old('publish', $exam->publish) ? 'checked' : '' }}>
