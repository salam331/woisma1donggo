...
@section('title', 'Rekap Kehadiran Kelas ' . $class->name . ' - Guru')
        ...
        <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm rounded-lg">
            <div class="flex justify-between items-center mb-6 p-6">
                <h2 class="font-semibold text-2xl text-gray-800 dark:text-gray-100">
                    Rekap Kehadiran Kelas {{ $class->name }} - Mata Pelajaran yang Anda Ajar
                </h2>
                ...
                <table class="min-w-full divide-y divide-gray-300 dark:divide-gray-700">
                    ...
                    <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                        ...
                        @php
                            ...
                            $subjectSummaries = [];
                            foreach($class->schedules->where('teacher_id', auth()->user()->teacher->id) as $schedule) {
                                $subjectId = $schedule->subject->id;
                                
                                $totalAttendances = \App\Models\Attendance::whereHas('schedule', function ($q) use ($subjectId, $class) {
                                    $q->where('subject_id', $subjectId)->where('class_id', $class->id);
                                })->count();

                                $presentCount = \App\Models\Attendance::whereHas('schedule', function ($q) use ($subjectId, $class) {
                                    $q->where('subject_id', $subjectId)->where('class_id', $class->id);
                                })->where('status', 'present')->count();

                                $absentCount = \App\Models\Attendance::whereHas('schedule', function ($q) use ($subjectId, $class) {
                                    $q->where('subject_id', $subjectId)->where('class_id', $class->id);
                                })->where('status', 'absent')->count();

                                $lateCount = \App\Models\Attendance::whereHas('schedule', function ($q) use ($subjectId, $class) {
                                    $q->where('subject_id', $subjectId)->where('class_id', $class->id);
                                })->where('status', 'late')->count();

                                $excusedCount = \App\Models\Attendance::whereHas('schedule', function ($q) use ($subjectId, $class) {
                                    $q->where('subject_id', $subjectId)->where('class_id', $class->id);
                        ...
                        @forelse($subjectSummaries as $subjectSummary)
                            <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">

                                <td data-label="Mata Pelajaran yang Anda Ajar" class="px-6 py-4 whitespace-nowrap">
                                ...
                                <td data-label="Aksi" class="px-6 py-4 whitespace-nowrap text-right">
                                    ...
                                    <div class="flex items-center justify-center gap-x-4">
                                        <a href="{{ route('guru.attendances.index-by-subject', ['classId' => $class->id, 'subjectId' => $subjectSummary['subject']->id]) }}"
                        ...
                        @empty
                            <tr>
                                <td colspan="8" class="px-6 py-4 text-center text-gray-500 dark:text-gray-300">
                                    Anda belum mengajar mata pelajaran di kelas {{ $class->name }} atau belum ada data kehadiran.
                                </td>
                            </tr>
                        @endforelse
