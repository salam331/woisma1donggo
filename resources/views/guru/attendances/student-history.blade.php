...
@section('title', 'Histori Absensi Siswa - Guru')
...
@section('content')
    ...
    <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-6 overflow-x-auto">
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
            ...
            <a href="{{ route('guru.attendances.index-by-subject', ['classId' => $student->class->id, 'subjectId' => $subject->id]) }}"
        ...
        <div class="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 mb-6">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-900 dark:text-gray-200">{{ $student->name }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">NIS</label>
                    <p class="mt-1 text-sm text-gray-900 dark:text-gray-200">{{ $student->nis }}</p>
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Kelas</label>
                    <p class="mt-1 text-sm text-gray-900 dark:text-gray-200">{{ $student->class->name ?? '-' }}</p>
                </div>
            </div>
        </div>
    ...
    <a href="{{ route('guru.attendances.show', ['attendance' => $attendance->id, 'subjectId' => $subject->id]) }}"
    ...
    <a href="{{ route('guru.attendances.edit', $attendance) }}"
    ...
    <form method="POST" action="{{ route('guru.attendances.destroy', $attendance) }}" class="inline">
        @csrf
        @method('DELETE')
        <button type="submit"
        ...
        <div class="mt-6 bg-gray-50 dark:bg-gray-700 rounded-lg p-4">
            <h4 class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Ringkasan Absensi</h4>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                ...
                @endphp
                <div>
                    <span class="text-green-600 dark:text-green-400 font-semibold">{{ $presentCount }}</span>
                    <span class="text-gray-600 dark:text-gray-400"> Hadir</span>
                </div>
                <div>
                    <span class="text-red-600 dark:text-red-400 font-semibold">{{ $absentCount }}</span>
                    <span class="text-gray-600 dark:text-gray-400"> Tidak Hadir</span>
                </div>
                <div>
                    <span class="text-yellow-600 dark:text-yellow-400 font-semibold">{{ $lateCount }}</span>
                    <span class="text-gray-600 dark:text-gray-400"> Terlambat</span>
                </div>
                <div>
                    <span class="text-blue-600 dark:text-blue-400 font-semibold">{{ $excusedCount }}</span>
            ...
            @if($totalCount > 0)
                <div class="mt-2 pt-2 border-t border-gray-200 dark:border-gray-600">
                    <span class="text-sm text-gray-600 dark:text-gray-400">
                        Total: {{ $totalCount }} kali absensi | 
                        Persentase Hadir: {{ round(($presentCount / $totalCount) * 100, 2) }}%
                    </span>
                </div>
            @endif
        </div>
